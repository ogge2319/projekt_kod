...
require([
    ...
    document.getElementById("savePoiButton").addEventListener("click", function () {
        ...
        var poiImage = document.getElementById("poiImage").value;

        if (poiName && clickedPoint) {
            ...
            infoTemplate.setTitle(poiName);
            var content = poiDescription;
            if (poiImage) {
                content = "<img src='" + poiImage + "' alt='" + poiName + "' style='width:100%;'><p>" + poiDescription + "</p>";
            }
            infoTemplate.setContent(content);

            customMarkerGraphic.setInfoTemplate(infoTemplate);
            map.graphics.add(customMarkerGraphic);

            // Göm formuläret efter att POI har lagts till
            hidePoiForm();

            // Ta bort klassen "clicked" från länken för att lägga till egna POI
            document.getElementById("addOwnPOILink").classList.remove("clicked");
        }
    });


    // Event listener för avbryt knappen
    document.getElementById("cancelPoiButton").addEventListener("click", function () {
        hidePoiForm();
    });

    document.getElementById("addOwnPOILink").addEventListener("click", function(event) {
        event.preventDefault(); // Förhindra standardbeteendet för länken (t.ex. att följa länken)
        // Töm inmatningsfälten för namn, beskrivning och bild-URL
        document.getElementById("poiName").value = "";
        document.getElementById("poiDescription").value = "";
        document.getElementById("poiImage").value = "";
        // Göm formuläret om det redan är synligt
        document.getElementById("poiForm").style.display = "none";
        // Ange att användaren har valt att lägga till egna POI
        addingOwnPOI = true;
    });
